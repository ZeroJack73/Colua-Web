/**
 * QR Code Generator ultra-ligero y autónomo para COLUA Web Digital
 * Genera matrices QR y las dibuja en Canvas / SVG sin dependencias externas.
 */
(function (global) {
  // Implementación simplificada y robusta de generación QR (Type 1-10 Byte Mode)
  ...
  // Reed-Solomon & QR Code Matrix Engine
  ...
  QRCodeModel.createData = function (typeNumber, errorCorrectLevel, dataList) {
    ...
    if (buffer.getLengthInBits() > totalDataCount * 8) {
      throw new Error("Datos demasiado grandes para el tamaño del QR");
    }
  ...
  // Renderizador en DOM (Canvas / SVG)
  ...
  QRCode.prototype.render = function () {
    ...
    var nModuleW = nWidth / nCount;
    var nModuleH = nHeight / nCount;
    ...
    for (var row = 0; row < nCount; row++) {
      for (var col = 0; col < nCount; col++) {
        if (this._oQRCode.isDark(row, col)) {
          ctx.fillRect(Math.round(col * nModuleW), Math.round(row * nModuleH), Math.ceil(nModuleW), Math.ceil(nModuleH));
        }
      }
    }
  ...
  // Helper estático para dibujar directamente a un contexto canvas existente
  QRCode.drawToCanvasContext = function (ctx, text, x, y, size, darkColor, lightColor) {
    try {
      ...
      ctx.fillStyle = darkColor || "#0f2252";
      for (var r = 0; r < count; r++) {
        for (var c = 0; c < count; c++) {
          if (qr.isDark(r, c)) {
            ctx.fillRect(Math.round(x + c * moduleSize), Math.round(y + r * moduleSize), Math.ceil(moduleSize), Math.ceil(moduleSize));
          }
        }
      }
    } catch (e) {
      console.error("Error dibujando QR en canvas:", e);
    }
  };

  // Helper estático para generar Data URL en base64 de forma síncrona
  QRCode.toDataURL = function (text, size, darkColor, lightColor) {
    try {
      var canvas = document.createElement('canvas');
      var s = size || 160;
      canvas.width = s;
      canvas.height = s;
      var ctx = canvas.getContext('2d');
      QRCode.drawToCanvasContext(ctx, text, 0, 0, s, darkColor || '#f59e0b', lightColor || '#071536');
